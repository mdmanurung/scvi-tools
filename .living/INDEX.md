...
| [learnings.md](learnings.md) | Gotchas, bugs, edge cases (L-001…L-021) |
...
| [findings/FINDINGS_REGISTRY.md](findings/FINDINGS_REGISTRY.md) | Empirical results registry (F-001…F-010) |
...
| lambda, ewc, continual | L-013, D-003, F-008 |
...
**Benchmark validity / publication gate** (L-010, L-011, D-002, D-007, F-001…F-009): All current findings are pre-publication. batch_size root cause, manifest-mode discipline, and full-cohort resubmits are the active gate.

**B1 Nuñez reversal** (F-003, todo Monitor-B1-reversal): CytoANVI Δ−0.013 on Nuñez r0.05 e1000 3-seed (confirmed, not just vignette). Interpretable: clean Nuñez labels + large homogeneous populations = strong kNN. Monitor on full cohort.
...
**Transductive evaluation footgun** (L-016): Three independent code paths had the same error — KNN imputation (A3), Nuñez annotation, B5 novelty detection. Pattern: any `fit()` call that includes test/query indices is transductive. Tripwire: assert held indices are absent from training set.
