...
### D-033 — [2026-07-12] Retrain MrTotalVI/MrMultiVI with sample_key='donor_timepoint' for valid DA
**Context**: The original training used `sample_key="donor"` (10 samples). `differential_abundance(sample_cov_keys=["timepoint"])` requires each sample to map to exactly one timepoint. With `sample_key="donor"`, each sample spans all 4 timepoints — the covariate assignment is undefined (drop_duplicates picks first-seen row by accident). MRVI was trained with `sample_key="final_label"` (donor×timepoint, 38 samples) and its DA succeeded; MrTotalVI/MrMultiVI DA was silently invalid.
**Decision**: Retrain both models with `sample_key="donor_timepoint"`. Filter training data to `modeling_keep_known_timepoint=True` (97,954 of 125,706 cells; 38 non-empty donor×timepoint combinations). Model name tag: `_dtp_s{N}`. Script: `train_multiseed.py --sample-key donor_timepoint`. New jobs: 25211393–25211398.
**Rationale**: `donor_timepoint` makes each sample a unique (donor, timepoint) pair, so `sample_cov_keys=["timepoint"]` is well-defined (each sample maps to exactly one timepoint). The `donor_key="donor"` within-donor centering in `_stats.differential_abundance` is then the correct variance-reduction step for this repeated-measures design.
**Consequences**: Old `_s{N}` models remain on disk for scIB comparison but their DE/DA results are invalid. New `_dtp_s{N}` models will need full downstream pipeline: scIB, DE with `sample_cov_keys=["timepoint"]`, DA with `sample_cov_keys=["timepoint"], donor_key="donor"`. The `_stats.py` validation improvement (L-077 structural mitigation) should also be added before the DA run.
**Status**: active

### D-034 — [2026-07-12] CRN fix for LFC: u_anchor optional param; legacy qu.mean path kept for standalone callers
**Context**: The CRN fix requires both x_0 and x_1 decode calls to share the same sampled u per MC draw. `compute_h_from_x_eps` is also called directly in tests (D-021 determinism check) without an MC context.
**Decision**: Add `u_anchor: torch.Tensor | None = None` to both `compute_h_from_x_eps` hooks. When provided: use it as the u for `qz(u, cf_sample)`. When None: fall back to `qu.mean` (legacy, Jensen-biased, but deterministic — keeps D-021 test intact). `_stats.py` always provides `u_anchor`.
...
### D-038 — [2026-07-12] freeze_prior_after_init: freeze prior location/shape, keep mixture weights trainable
**Context**: DA stability analysis showed cross-seed variance in `differential_abundance` comes entirely from the u-encoder converging to different configurations per seed — not from inference-time stochasticity. Two candidate fixes were evaluated: (a) MC marginalization (`n_mc_samples`) and (b) freezing the prior after initialization.
**Decision**: Implement only `freeze_prior_after_init: bool = False` in `init_u_prior` / both `_setup_hierarchy` / both model constructors. Drop MC marginalization.
**Rationale**: MC marginalization was incorrectly framed as reducing cross-seed variance. It actually corrects the Jensen gap (`log p(E[u]) ≠ E[log p(u)]`), a second-order bias. More importantly, switching from `give_mean=True` to `give_mean=False` introduces inference-RNG non-determinism that does not exist today — a regression for the stated goal. `freeze_prior_after_init` is the direct fix: it pins the prior landscape so the u-encoder optimizes against the same attractor basin every seed. The freeze only helps when paired with a data-driven prior (`init_prior_from_data=True` + `u_prior="vamp"`) — freezing a `randn`-initialized prior locks in a different random landscape per seed. `u_prior_logits` (mixture weights) is kept trainable in all cases so the model can adapt which attractor components are used.
**Consequences**: For MrTotalVI-LN (default MoG prior), `freeze_prior_after_init=True` alone provides no benefit. The full seed-stability path is: `u_prior="vamp"` + `init_prior_from_data=True` (k-means centroids, `random_state=0`) + `freeze_prior_after_init=True`. This is not the default — users must opt in explicitly.
**Status**: active

### D-039 — [2026-07-12] n_mc_samples in differential_abundance: Jensen-gap bias correction, not seed variance reduction
**Context**: After implementing `freeze_prior_after_init` (D-038), user requested "Implement all" — meaning also implement `n_mc_samples` MC marginalization in `differential_abundance`.
**Decision**: Implement `n_mc_samples: int = 1` in `_stats.py:differential_abundance` and thread through both model wrappers. Default `1` preserves exact existing behavior. When `n_mc_samples > 1`, draw K samples from `q(u|x)` with `give_mean=False` and return `logsumexp(log_probs_k) - log(K)` = `log(1/K * Σ_k p(u_k | sample))`.
**Rationale**: This corrects the Jensen-gap bias: the plug-in estimator `log p(E[u] | sample)` underestimates `log E_{q(u|x)}[p(u | sample)]` by Jensen's inequality. The MC estimate is the correct quantity. Documentation explicitly states it does NOT reduce cross-seed variance — that variance originates from training dynamics, not the evaluation estimator. Inference-time stochasticity is introduced for `n_mc_samples > 1`; callers should fix `scvi.settings.seed` for reproducibility.
**Consequences**: Default (`n_mc_samples=1`) is fully deterministic and bit-for-bit identical to the previous behavior. Non-default values trade compute (K extra forward passes through `get_latent_representation`) for a tighter log-probability estimate.
**Status**: active
...
**Cross-reference (2026-08-06)**: `benchmarks/ANALYSIS_MANIFEST.md`'s "Adversarial findings" table lists this same D-041 decision as **REFUTED FOR PROMOTION**. This is not a contradiction with the CONFIRMED status above — the two verdicts answer different questions. This entry's CONFIRMED status is about whether the underlying measurement is real: the artifact was located and the std=0.192 result (passing the pre-registered std ≤ 0.30 + all-seeds-positive criterion) stands. The manifest's REFUTED FOR PROMOTION verdict is about a stricter, separate bar — whether this is ready to promote into a shipped default — and finds it is not, because the located artifacts lack data/feature/environment/config/artifact hashes and came from a dirty checkout, i.e. they don't yet meet the immutable-reproduction evidence standard. Per `todo/TODO_REGISTRY.md` (P2-006), the currently-tracked blocker on that promotion path is RDX-03-11. Note the manifest cites the artifact location as `outputs/validate_freeze_prior/`, distinct from the `.scratch/mr-schisto-benchmark/results/...` path cited above; whether these are the same underlying evidence has not been verified here and is not resolved by this note. Read both entries together: CONFIRMED = the effect is real; REFUTED FOR PROMOTION = not yet safe to ship as a default.
