...
### L-094 — [2026-07-27] Variance-fraction metrics (eta^2, R^2) cannot compare representations that differ in total variance
**Category**: gotcha
**Tags**: mrtotalvi, totalvi, latent-representation, metrics, effective-rank, review
**mitigation_type**: ambient-awareness
**structural_mitigation_candidate**: knn_state_recovery.py
**Body**: Multivariate eta^2 = trace(between)/trace(total) was used to argue MrTotalVI's low-rank z is "more state-informative" than stock TOTALVI's (0.614 vs 0.368 on the RDX-03 canonical_human fixture). The metric is rotation-invariant, which is true but answers the wrong objection: invariance holds *within* one embedding's basis, not *across* two embeddings whose denominators differ. Since the compression under study is precisely a shrinkage of non-state variance, eta^2 rises mechanically — collapse every cell onto its state centroid and eta^2 -> 1 at vanishing rank. A denominator-free retest (stratified 5-fold kNN macro-F1, k=15, same held-out cells) gave a materially different answer: MrTotalVI z 0.798 raw / 0.843 z-scored vs stock TOTALVI 0.811 / 0.799 — comparable, not superior — while MrTotalVI u at rank 5.76 was genuinely worse (0.751). Rule: when comparing embeddings for information content, use a recovery measure (kNN label F1, linear probe) not a variance-share measure. Artifacts in `.scratch/mrtotalvi-z-vs-totalvi/`.

### L-095 — [2026-07-27] labels_key silently makes MrTotalVI's MoG prior label-supervised
**Category**: gotcha
**Tags**: mrtotalvi, mog-prior, labels, benchmarking, double-dipping
**mitigation_type**: ambient-awareness
**structural_mitigation_candidate**: test_mrtotalvi_label_conditioned_mog_prior
**Body**: Registering `labels_key` in `MrTotalVI.setup_anndata` does more than record metadata. `_components.py:283` sets `resolved_k = n_labels if n_labels > 1`, which makes the MoG mixture count equal the label count and thereby activates the label-conditioned logit offset in `build_u_prior` (`u_prior_label_weight`, default **10.0**) — each cell's KL(q_u||p_u) is evaluated against a prior component picked by its ground-truth label. The schisto `mrtotalvi_ln` benchmark models (`train_multiseed.py:138-150`) registered `labels_key="celltype"` while the stock TOTALVI comparator (`train_model.py:230-235`) did not, so every cell-type-alignment comparison between them (scIB bio-conservation, per-PC eta^2_celltype) is supervised-vs-unsupervised on the supervised label. Before benchmarking any embedding on a label-based metric, check whether that label entered training. RDX-03's `convergence_runner.py:535-542` registers no `labels_key` and is unaffected.

### L-096 — [2026-07-27] Multigrate MultiVAE diverges to NaN on some seeds with its default settings
**Category**: gotcha
**Tags**: multigrate, benchmarking, numerical-stability, cite-seq, comparator
**mitigation_type**: ambient-awareness
**structural_mitigation_candidate**: .scratch/mrtotalvi-z-vs-totalvi/train_multigrate.py
**Body**: Training `multigrate.model.MultiVAE` (1.0.1, z_dim=20, losses=['nb','nb'], integrate_on='donor') on 97,954 schisto human CITE-seq cells diverged on 1 of 3 seeds: seed 2 produced all-NaN `loc` for the latent Normal at epoch 185 and raised `ValueError: Expected parameter loc ... to satisfy the constraint Real()`, killing the run. Seeds 0 and 1 completed normally (effective rank 10.67 and 10.03, early-stopped ~8-9 min each) with the model's own default `lr=5e-4`/`batch_size=256` — no retuning was applied. Stock TOTALVI and MrTotalVI trained 3/3 seeds on the identical cell set without incident, so this is specific to Multigrate rather than the data. Consequences for benchmarking: (1) budget for seed failures and never report a silently-reduced seed count as a clean run; (2) a substituted seed is a selection step and must be disclosed (here seed 3 replaced seed 2, with the divergence itself reported as an outcome); (3) if Multigrate is needed as a stable comparator, gradient clipping or a lower lr would have to be justified as a deviation from its published defaults.

### L-097 — [2026-07-28] Comparing embeddings at a single Leiden resolution produces spurious findings
**Category**: gotcha
**Tags**: clustering, leiden, benchmarking, embeddings, mrtotalvi, methodology
**mitigation_type**: ambient-awareness
**structural_mitigation_candidate**: .scratch/mrtotalvi-z-vs-totalvi/resolution_sweep.py
**Body**: Scoring MrTotalVI-LN MoG `u` vs VampPrior `u` by best-matching-cluster F1 at Leiden resolution 1.0 produced two confident and WRONG conclusions: that plasma cells (n=126) are lost under VampPrior (F1 0.153 vs MoG 0.580) and that CD16- NK cells (n=434) are recovered only by VampPrior (0.563 vs 0.252). Sweeping 0.5/1.0/2.0/4.0/6.0 dissolved both. Plasma cells under VampPrior need resolution >=2.0, where VampPrior then BEATS MoG (best 0.823 vs 0.745); CD16- NK under MoG needs 2.0, where it matches VampPrior (0.555 vs 0.563). At resolution 1.0 the two embeddings simply sat at different points on their resolution curves, in opposite directions. Different embeddings reach peak recovery at different resolutions — a fixed resolution measures where each happens to sit on its curve, not which resolves cell types better. Always report best-achievable F1 across a resolution sweep, and note that seed variance at a single resolution (MoG plasma 0.580 +/- 0.209) is itself partly an artefact of straddling a curve's steep region. Related: [[L-094]] (variance-fraction metrics), which was the same class of error — an operating-point/denominator artefact mistaken for a biological result.
