...
| B5: Novelty detection | `run.py --task b5` | AUROC (mean over types is PRIMARY) | CytoVI kNN-distance OOD | roider-full 3-seed ✅ COMPLETE (jobs 25145151/52/53); CytoANVI-latent kNN OOD diagnostic NOT YET RUN — jobs 25149032/33/34 were B5 sweep reruns (confirmed 2026-07-07), not the kNN diagnostic | **FULL-COHORT (FINAL — NEGATIVE RESULT):** mean_auroc **0.484±0.019 (below chance)** vs CytoVI kNN-distance OOD baseline **0.775±0.002**. CytoANVI TTA-uncertainty is far worse than plain kNN-distance in CytoVI latent. See F-013. best_auroc 0.833±0.122 (e1000 subset max/cherry-picked) is NOT the headline. CytoANVI-latent kNN OOD diagnostic has NOT run yet — jobs 25149032/33/34 were confirmed as B5 sweep reruns. TTA-method weakness vs latent-space weakness remains unresolved. |
...
| B9: mapQC | `run.py --task b9` | mapqc_score | Low control | Nuñez s0 ✅ COMPLETE (job 25170524, 2026-07-07); Roider B9 NOT YET RUN | **Nuñez s0 (job 25170524):** label transfer accuracy **0.947**, macro-F1 **0.887** (strong inductive transfer). mapqc result: n_query_controls=24,972; **n_pass=0** (all query controls fail at score_threshold=2.0, frac_dist_to_ref=null). The n_pass=0 means no query cells pass the neighborhood-density quality filter — possible causes: (1) threshold 2.0 too strict for this dataset, (2) query distribution genuinely does not overlap reference neighborhoods. Label transfer accuracy suggests the latent space is coherent; the mapqc failure is specific to the density filter. **Roider B9 not yet run.** See F-039. |
