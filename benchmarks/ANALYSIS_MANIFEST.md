...
| B9: mapQC | `run.py --task b9` | mapqc_score | Low control | **Nuñez 3-seed ✅ COMPLETE** (jobs 25170524/25211797/25211798); **Roider-full ❌ BLOCKED** (job 25211799 FAILED, upstream mapQC bug) | **Nuñez 3-seed:** label transfer acc **0.952±0.005**, macro-F1 **0.904±0.015** (strong inductive transfer). mapqc **n_pass=0** in all 3 seeds (~25k query controls/seed, threshold=2.0, frac_dist_to_ref=null) — neighborhood-density filter rejects all query controls despite good label transfer. **Roider BLOCKED:** `ValueError: Category 31 not found in source registry` at `transfer_field` — Roider query has a Leiden category absent from training registry; upstream mapQC code does not pass `extend_categories=True`. Not fixable in CytoANVI. See F-039. |
