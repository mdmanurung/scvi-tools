...
| B9: mapQC | `run.py --task b9` | mapqc_score | Low control | **Nuñez 3-seed ✅ COMPLETE** (jobs 25170524/25211797/25211798); **Roider-full ⏳ registry bug FIXED 2026-08-06, full-cohort re-run pending** (job 25211799 was the failing run under the pre-fix code) | **Nuñez 3-seed:** label transfer acc **0.952±0.005**, macro-F1 **0.904±0.015** (strong inductive transfer). mapqc **n_pass=0** in all 3 seeds (~25k query controls/seed, threshold=2.0, frac_dist_to_ref=null) — neighborhood-density filter rejects all query controls despite good label transfer. **Roider (was BLOCKED, now fixed in CytoANVI):** `ValueError: Category 31 not found in source registry` at `transfer_field` — root cause was `build_mapqc_anndata` embedding `ref`/`query` AnnData that were never registered with the model's AnnDataManager, not an upstream mapQC limitation. Fixed via explicit `source_registry=`/`extend_categories=True` registration + out-of-registry label remap on a scratch copy (`src/cytoanvi/mapping_qc.py::_embed_foreign_adata`); unit-tested (`tests/cytoanvi/test_mapping_qc_mock.py::test_build_mapqc_anndata_handles_label_category_absent_from_reference`). Full-cohort Roider B9 SLURM re-run not yet repeated. See F-039 (corrected 2026-08-06). |
...
| Claim | Verdict |
|---|---|
| D-041: VampPrior+frozen → 18.7% DA variance reduction | **REFUTED FOR PROMOTION** — artifacts were later located under `outputs/validate_freeze_prior/`, but they lack data/feature/environment/config/artifact hashes and were produced from a dirty checkout; C1 remains an unvalidated candidate pending immutable reproduction |
...
- **Internal-use review** agrees with publication review verdicts and adds: (1) MrTotalVI DA is Not Ready even internally; (2) five P0 infra fixes were needed; (3) D-041 is refuted for promotion until immutable reproduction
