...
def _load(dataset: str, data_dir: str, leiden_resolution: float = 0.05, max_cells: int = 100_000):
    if dataset == "synthetic":
        merged, p1, _ = data_mod.make_synthetic_panels()
        return p1
    if dataset == "nunez":
        return data_mod.load_nunez(
...
def main():
    ...
    ap.add_argument("--preproc-schemes", default="minmax,zscore,rank", help="A2: comma-separated")
    ...
    ap.add_argument("--leiden-resolution", type=float, default=0.05)
    ap.add_argument("--n-posterior-samples", type=int, default=50, help="A3: posterior draws")
    ...
    schemes = tuple(s.strip() for s in args.preproc_schemes.split(",") if s.strip())
