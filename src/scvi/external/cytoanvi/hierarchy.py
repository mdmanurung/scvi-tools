...
Requires ``pip install scvi-tools[cytoanvi-hierarchy]`` (scHPL). Not exported from the top-level
``cytoanvi`` package; import from ``scvi.external.cytoanvi.hierarchy``.
"""
...
if TYPE_CHECKING:
    ...
    from scvi.external.cytoanvi._model import CytoANVI
...
def _representative_schpl_node(
    ...
    if len(leaf_matches) > 1:
        raise ValueError(
            ...
            f"{[leaf.name[0] for leaf in leaf_matches]}"
        )
    if len(leaf_matches) == 1:
...
def reachability_from_schpl_tree(
    ...
    from scvi.external.cytoanvi._hce import validate_reachability_matrix

    root = _get_tree_root(tree)
    label_names = list(model_labels)
    schpl_leaves = [leaf.name[0] for leaf in root.get_leaves()]
...
def run_tree_arches_pipeline(
    ...
    **kwargs: Any,
    ...
    if query_adata is None and reference_adata is not None and combined_latent is None and combined_adata is None:
        raise ValueError(
